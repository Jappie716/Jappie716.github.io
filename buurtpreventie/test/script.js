...
function loadMap() {
    onSnapshot(collection(db, "users"), (snapshot) => {
        ...
        // Vaste gebouwen eerst toevoegen
        const buildingsHTML = `
            <div class="building bingo" onclick="window.location.href='bingo.html'">🎰<span>Bingo Club</span></div>
        ...
        houses.forEach(({ uid, data, isMe }) => {
            ...
            tooltip.className = 'house-tooltip hidden';
            tooltip.innerHTML = `
                <strong>${data.username}</strong>
                <div class="tooltip-address">🏠 Huisnummer: ${data.huisnummer || 'Onbekend'}</div>
                <div class="tooltip-status">${data.status || 'Status onbekend'}</div>
            `;
            ...
            b.addEventListener('mouseenter', (e) => {
                ...
                tooltip.style.left = (rect.left - mapRect.left + 10) + 'px';
                tooltip.style.top = (rect.top - mapRect.top - 80) + 'px';
            });
